...
def Extract_Person_Events_BR(zone):
	...
	dbHandler = Database_Handler.Database_Handler(DATABASE_HOST,PORT, USERNAME, PASSWORD, "Building_Robotics_Person_Pattern_Base")		
...
def Extract_Temporal_Cluster_Person_BR(user): # The number of minutes a new event can differ from mean of previous events (5-minute increments)
		
	event_dbHandler = Database_Handler.Database_Handler(DATABASE_HOST,PORT, USERNAME, PASSWORD, "Building_Robotics_Person_Pattern_Base")
	
	activities = ['Too Hot','Too Cold','Comfortable','Cancel','First Interaction','Last Interaction']	
	
	result = event_dbHandler.select("SELECT DISTINCT zone_id FROM Events_BR WHERE originating_user = %i ORDER BY zone_id" % (user))
			...
			try:
				...
				for outer_threshold in range(1,8):
					for inner_threshold in range(1,8):
						...
						result = event_dbHandler.select("SELECT * FROM Events_BR WHERE activity = '%s' AND zone_id = %i AND originating_user = %i ORDER BY time_index" % (activity,location,user))
				...
				event_result = event_dbHandler.select("SELECT * FROM Events_BR WHERE originating_user = %i AND activity = '%s'" % (user,activity))
				...
				for event in event_result:			
					for i in range(len(best_cluster)):				
						try:
							...
							current_event[18] = Get_Cluster_ID(activity,location,i+1)
...
def Extract_Singular_Patterns_BR(user):
	...
	event_dbHandler.update("INSERT INTO Events_BR_final_tmp SELECT * FROM Events_BR_final WHERE originating_user = %i" % (user))
...
def Map_Singular_Patterns_BR():
	...
	for user in user_ids:
		...
		result = dbHandler.select("SELECT * FROM Events_BR_final WHERE originating_user = %i ORDER BY timestamp" % (user))
...
def Extract_Conditional_Patterns(zone,max_interval_length,pool):
	...
	dbHandler.update("INSERt INTO Events_BR_mapped_tmp SELECT * FROM Events_BR_mapped WHERE originating_user = %i" % (zone))	
	...
	for pattern_length in range(2,10):						
		...
		if pattern_length == 2:
			...
			for k in range(len(singular_pattern_id)):
				result = dbHandler.select("SELECT count(id) FROM Events_BR_mapped_tmp WHERE originating_user = %i AND INSTR(singular_pattern_id,',%s,') > 0" % (zone,singular_pattern_id[k]))
...
def Calculate_Probability(pattern_combinations,pattern_combinations_temporal_intervals,pattern_combinations_temporal_specificity,pattern_combinations_count,pattern_length,user,lower,upper,months,weeks,dates,timestamps,temporal_interval_description):
	...
	#dbHandler_pattern = Database_Handler.Database_Handler(DATABASE_HOST,PORT, USERNAME, PASSWORD, "Building_Robotics_Person_Pattern_Base")
	dbHandler = Database_Handler.Database_Handler(DATABASE_HOST,PORT, USERNAME, PASSWORD, "Building_Robotics_Person_Pattern_Base")
...
def Clean_Complaints_Data():
	...
	dbHandler_1 = Database_Handler.Database_Handler(DATABASE_HOST,PORT, USERNAME, PASSWORD, "Building_Robotics_Person_Pattern_Base")
...
def Update_Specificity():
	...
	dbHandler.update("UPDATE Singular_Pattern_Base SET specificity = 0 WHERE dow IS NULL AND dom IS NULL AND season IS NULL AND week_weekend IS NULL AND number_in_month IS NULL AND temperature IS NULL AND blast_action IS NULL")
	dbHandler.update("UPDATE Singular_Pattern_Base SET specificity = 1 WHERE dow IS NULL AND dom IS NULL AND season IS NULL AND week_weekend IS NULL AND number_in_month IS NULL AND temperature IS NULL AND blast_action IS NOT NULL")
	dbHandler.update("UPDATE Singular_Pattern_Base SET specificity = 2 WHERE dow IS NULL AND dom IS NULL AND season IS NULL AND week_weekend IS NOT NULL AND number_in_month IS NULL AND temperature IS NULL AND blast_action IS NULL")
	dbHandler.update("UPDATE Singular_Pattern_Base SET specificity = 3 WHERE dow IS NULL AND dom IS NULL AND season IS NOT NULL AND week_weekend IS NULL AND number_in_month IS NULL AND temperature IS NULL AND blast_action IS NULL")
	dbHandler.update("UPDATE Singular_Pattern_Base SET specificity = 4 WHERE dow IS NOT NULL AND dom IS NULL AND season IS NULL AND week_weekend IS NULL AND number_in_month IS NULL AND temperature IS NULL AND blast_action IS NULL")
	dbHandler.update("UPDATE Singular_Pattern_Base SET specificity = 5 WHERE dow IS NOT NULL AND dom IS NULL AND season IS NULL AND week_weekend IS NULL AND number_in_month IS NOT NULL AND temperature IS NULL AND blast_action IS NULL")
	dbHandler.update("UPDATE Singular_Pattern_Base SET specificity = 6 WHERE dow IS NULL AND dom IS NOT NULL AND season IS NULL AND week_weekend IS NULL AND number_in_month IS NULL AND temperature IS NULL AND blast_action IS NULL")	
...
if __name__ == "__main__":
	...
	result = dbHandler.select("SELECT user_id,count(id) FROM complaints GROUP BY user_id ORDER BY count(id) DESC")
	
	zones = []
	
	for row in result:		
		#Extract_Temporal_Cluster_Person_BR(int(row[0]))
